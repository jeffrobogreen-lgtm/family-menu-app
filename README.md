...
- **`prisma` and `@prisma/client`/`@prisma/adapter-pg` are pinned to matching exact versions (`7.10.0`)**, not `"latest"` — found the hard way on 2026-08-27, when `"latest"` resolved the `prisma` CLI package to an unreleased `8.0.0-rc.12` pre-release while `@prisma/client` stayed on stable `7.10.0`. The mismatched pair broke `prisma generate` during Vercel's build (`postinstall`) with a cryptic `CLI.UNKNOWN_COMMAND` error, since the rc CLI didn't have `generate` wired up the same way yet. If you bump these later, bump `prisma`, `@prisma/client`, and `@prisma/adapter-pg` together to the same version.
